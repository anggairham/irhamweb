...
async function parse(markdown) {
  ...
  const md = new MarkdownIt({
    ...
    highlight: (code, lang, lineHightlight) => {
      ...
      if (lineHightlight) {
        const cleanLineString = lineHightlight
          .replace("{", "")
          .replace("}", "");
        ...
        lines.forEach((line) => {
          ...
          if (lineMultiple.length > 1) {
            const lineRange = range(
              parseInt(lineMultiple[0], 10),
              parseInt(lineMultiple[1], 10)
            );
  ...
  md.renderer.rules.image = function (tokens, idx) {
    ...
    const nextImage2x = `/_next/image?url=${encodeURIComponent(
      src
    )}&w=1200&q=75`;
    const nextImage1x = `/_next/image?url=${encodeURIComponent(
      src
    )}&w=768&q=75`;
